...
tower_img = pygame.image.load("tower (2).png")  # Загружаем изображение башни
...
Money_img = pygame.image.load("money.png")  # Загружаем изображение денег
...
class Ship:  # Создаем новый класс для отображеня картинки корабля
    def __init__(self):
    ...
    def draw(self, win):  # Определяем метод draw, который рисует корабль на заданном экране
        win.blit(ship_img, (self.x, self.y))  # Рисуем изображение корабля на заданном экране в заданных координатах
...
class Enemy:
    ...
    def draw(self, win):  # Определяем метод, который рисует врага на заданном экране
        ...
        pygame.draw.rect(win, RED,
                         (center_x - 10, self.y - 10, 20, 5))  # Рисуем красный прямоугольник как полоску здоровья врага
...
class Bullet:
    def __init__(self, x, y, target_x,
        ...
        self.speed = 5  # Задаем скорость снаряда
        self.damage = 1  # Задаем урон снаряда
    ...
    def draw(self, win):  # Определяем метод, который рисует снаряд на экране
        if self.is_active:  # Проверяем, активен ли снаряд
...
class Tower:
        ...
        if self.timer % 180 == 0:
            ...
            if closest_enemy:  # Если враг найден, то
                ...
                if closest_enemy.health <= 0:
                    ...
                    self.shots_fired = 0
                    global BUDGET
                    BUDGET += 100  # Добавляем бонус к бюджету
    ...
    def draw(self, win):
        # Метод для отрисовки башни на экране
...
def loading_screen():
    ...
    towers = []
    bullets = []
    castle = Castle(770, 135)  # Создаем объект замка
    ...
    time_start = None
    while run:  # Пока переменная run равна True, цикл будет продолжаться
